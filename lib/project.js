      ...
      try {
        ...
        function onRename(args) {
          ...
          if (newTitle !== oldTitle) {
            ...
            if (target.cliSessionId && typeof adapter.renameSession === "function") {
        ...
        function onMarkDone(args) {
          ...
          var oldTitle = target.title || "";
          var hasPrefix = oldTitle.indexOf(DONE_PREFIX) === 0;
          var newTitle;
          var wasAlreadyDone = false;
          if (args && args.undo) {
            newTitle = hasPrefix ? oldTitle.substring(DONE_PREFIX.length) : oldTitle;
          } else {
            if (hasPrefix) { newTitle = oldTitle; wasAlreadyDone = true; }
            else { newTitle = DONE_PREFIX + oldTitle; }
          }
          if (newTitle !== oldTitle) {
            target.title = newTitle.substring(0, 100);
            target.titleManuallySet = true;
            sm.saveSessionFile(target);
            sm.broadcastSessionList();
            if (target.cliSessionId && typeof adapter.renameSession === "function") {
              adapter.renameSession(target.cliSessionId, target.title, { dir: cwd }).catch(function (e) {
                console.error("[mark_session_done] SDK renameSession failed:", e.message || e);
              });
            }
          }
          return Promise.resolve({

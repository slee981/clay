    ...
    if (!isMate) {
      try {
        ...
        var spawnToolDefs = spawnMcp.getToolDefs(function onSpawn(spawnArgs) {
          ...
          // --- TUI path: register the session NOW, defer the `claude` PTY ---
          // launch until the user first opens the session. The deferred launch
          // happens in project-sessions.js switch_session, which has an
          // attached client ws to bind as the PTY owner. Launching here with
          // no attached client tends to make `claude` exit immediately, which
          // would tear the session record down again.
          if (mode === "tui") {
            ...
            return Promise.resolve({ sessionId: newSess.localId, title: newSess.title, mode: "tui" });
          }
          ...
          return Promise.resolve({ sessionId: newSess.localId, title: newSess.title, mode: "gui" });
        });

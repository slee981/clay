        ...
        var spawnToolDefs = spawnMcp.getToolDefs(function onSpawn(spawnArgs) {
          ...
          if (mode === "tui") {
            ...
            var cmd = "claude --session-id " + spawnedSid +
              ...
              " --permission-mode " + tuiMcpForSpawn.shellQuote(spawnArgs.permissionMode || "plan") +
              " --effort " + tuiMcpForSpawn.shellQuote(spawnArgs.effort || "high");
            ...
            var spawnedTerm = tm.create(80, 24, null, null, {
              ...
              onExit: function () {
                ...
                // Keep the session record. If claude exited (after running
                // the initial prompt, or any other reason), the conversation
                // is persisted at ~/.claude/projects/<cwd>/<uuid>.jsonl and
                // the next open will spawn `claude --resume <uuid>` via the
                // switch_session deferred-launch path.

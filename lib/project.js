    ...
    if (!isMate) {
      try {
        ...
        var spawnToolDefs = spawnMcp.getToolDefs(function onSpawn(spawnArgs) {
          ...
          var title = String(spawnArgs.title).substring(0, 100);

          // --- TUI path: launch claude CLI in an xterm with --mcp-config + prompt ---
          if (mode === "tui") {
            if (!tm) {
            ...
            var spawnedTerm = tm.create(80, 24, null, null, {
              ...
              onExit: function () {
                ...
                if (sm.sessions.has(spawnedLocalId)) {
                  try { sm.deleteSessionQuiet(spawnedLocalId); } catch (e) {}

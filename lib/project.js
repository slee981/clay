...
function createProjectContext(opts) {
  ...
  sm._savedDefaultEffort = (_projEffort && _projEffort.effort) || (_srvEffort && _srvEffort.effort) || null;
  sm.currentEffort = sm._savedDefaultEffort || "medium";
  ...
  var mcpServers = (function () {
    ...
    // Sessions MCP server: lets an agent spawn new sessions inside this
    // project and seed each with an initial user message. Main project
    // sessions only — mates run with a narrower tool surface.
    if (!isMate) {
      try {
        var spawnMcp = require("./spawn-mcp-server");
        var tuiMcpForSpawn = require("./tui-mcp-config");
        // Pull the first positional argument out of a leading slash command
        // (e.g. "/jira GP-222" -> "GP-222"). When the prompt starts with a
        // slash command, that argument is almost always the right session
        // title, regardless of what the calling agent passed. Falls back to
        // the agent-supplied title if no slash-arg can be extracted.
        function deriveTitleFromPrompt(prompt) {
          if (!prompt) return null;
          var m = /^\s*\/[A-Za-z0-9_-]+\s+(\S+)/.exec(prompt);
          return m ? m[1].substring(0, 100) : null;
        }
        function resolveTargetSession(args) {
          var sessionId = (args && typeof args.sessionId === "number") ? args.sessionId : null;
          var callingCliSessionId = (args && args.__callingCliSessionId) || null;
          if (sessionId != null) {
            var bySid = sm.sessions.get(sessionId);
            if (bySid) return bySid;
          }
          if (callingCliSessionId && typeof sm.findSessionByCliSessionId === "function") {
            var byCli = sm.findSessionByCliSessionId(callingCliSessionId);
            if (byCli) return byCli;
          }
          return sm.getActiveSession();
        }
        function onRename(args) {
          var target = resolveTargetSession(args);
          if (!target) {
            return Promise.reject(new Error(
              "No session to rename — pass session_id explicitly or call from inside a session."
            ));
          }
          var oldTitle = target.title || "";
          var newTitle = String(args.title || "").trim().substring(0, 100);
          if (!newTitle) return Promise.reject(new Error("title must be non-empty"));
          var titleChanged = (newTitle !== oldTitle);
          if (titleChanged) {
            target.title = newTitle;
            target.titleManuallySet = true;
          }
          // Optional kind update. Empty string clears the field; undefined
          // means "don't touch kind" so a plain rename doesn't wipe it.
          var kindChanged = false;
          if (typeof args.kind === "string") {
            var nextKind = args.kind ? args.kind.substring(0, 32) : "";
            if ((target.kind || "") !== nextKind) {
              target.kind = nextKind || undefined;
              kindChanged = true;
            }
          }
          ...
          if (titleChanged && target.cliSessionId && typeof adapter.renameSession === "function") {
            adapter.renameSession(target.cliSessionId, target.title, { dir: cwd }).catch(function (e) {
              console.error("[rename_session] SDK renameSession failed:", e.message || e);
            });
          }
          return Promise.resolve({
            sessionId: target.localId,
            cliSessionId: target.cliSessionId || null,
            oldTitle: oldTitle,
            newTitle: target.title,
            kind: target.kind || null,
          });
        }
        function onMarkDone(args) {
          var target = resolveTargetSession(args);
          if (!target) {
            return Promise.reject(new Error(
              "No session to mark — pass session_id explicitly or call from inside a session."
            ));
          }
          var wantDone = !(args && args.undo);
          var wasAlready = !!target.done === wantDone;
          target.done = wantDone;
          // Marking a session done clears its bookmark — favorites are
          // for in-flight work to keep at the top, so a finished session
          // belongs out of the favorites strip. Unmarking doesn't
          // restore the bookmark (the user can re-add manually).
          var clearedBookmark = false;
          if (wantDone && target.bookmarked) {
            target.bookmarked = false;
            target.favoriteOrder = null;
            clearedBookmark = true;
          }
          // Done means the underlying work is finished — also shut down
          // the live claude PTY for TUI sessions so it stops occupying
          // a terminal slot. Soft-exit first (send /exit so claude
          // saves its jsonl cleanly), then force-close after a short
          // grace period in case claude is mid-stream. The user can
          // still re-open the session later via `claude --resume`.
          if (wantDone && target.terminalId != null && tm) {
            try { tm.write(target.terminalId, "/exit\r"); } catch (e) {}
            var _doneTid = target.terminalId;
            setTimeout(function () {
              try { tm.close(_doneTid); } catch (e) {}
            }, 5000);
          }
          if (!wasAlready || clearedBookmark) {
            sm.saveSessionFile(target);
            sm.broadcastSessionList();
          }
          return Promise.resolve({
            sessionId: target.localId,
            cliSessionId: target.cliSessionId || null,
            // Kept for backwards compat with the tool's response shape;
            // the title itself no longer changes when marking done.
            oldTitle: target.title,
            newTitle: target.title,
            wasAlreadyDone: wasAlready && wantDone,
            done: target.done,
          });
        }
        function onRequestAttention(args) {
          var target = resolveTargetSession(args);
          if (!target) {
            return Promise.reject(new Error(
              "No session to flag — pass session_id explicitly or call from inside a session."
            ));
          }
          if (args && args.clear) {
        ...
        function onSendMessage(args) {
          var targetId = args && args.targetSessionId;
          if (typeof targetId !== "number") {
            return Promise.reject(new Error("targetSessionId must be a number"));
          }
          var target = sm.sessions.get(targetId);
          if (!target) {
            return Promise.reject(new Error("No session with localId " + targetId));
          }
          // Reject self-send so a runaway agent can't loop on itself.
          var callerCliSid = (args && args.__callingCliSessionId) || null;
          if (callerCliSid && target.cliSessionId === callerCliSid) {
            return Promise.reject(new Error("Cannot send a message to the session that's calling the tool"));
          }
          var rawText = String(args.text || "");
          if (!rawText) return Promise.reject(new Error("text must be non-empty"));
          // Auto-prefix the message with the sender's session title so the
          // recipient can tell where it came from. The sender knows their
          // own context; the recipient otherwise has no signal that this
          // message arrived from another agent rather than the human user.
          var senderTitle = "another session";
          if (callerCliSid && typeof sm.findSessionByCliSessionId === "function") {
            var senderSess = sm.findSessionByCliSessionId(callerCliSid);
            if (senderSess && senderSess.title) senderTitle = senderSess.title;
          }
          var text = "[from " + senderTitle + "]\n" + rawText;
          var mode = target.mode || "gui";

          if (mode === "tui") {
            // TUI: write to the live PTY so claude reads it as user input.
            // If no live terminal, we can't deliver — caller should ask the
            // user to open the session.
            if (typeof target.terminalId === "number" && tm) {
              try {
                // Trailing carriage return so claude treats it as a
                // submitted message rather than text the user is still
                // composing.
                tm.write(target.terminalId, text + "\r");
                return Promise.resolve({
                  targetSessionId: target.localId,
                  targetCliSessionId: target.cliSessionId || null,
                  mode: "tui",
                  delivered: true,
                  deferred: false,
                });
          ...
          // GUI: push into the session's history + the SDK message queue.
          // Mirrors what the scheduled-message handler in project.js does.
          var userMsg = { type: "user_message", text: text, _ts: Date.now() };
          target.history.push(userMsg);
          sm.appendToSessionFile(target, userMsg);
          sendToSession(target.localId, userMsg);
          if (!target.isProcessing) {
            target.isProcessing = true;
            onProcessingChanged();
            target.sentToolResults = {};
            sendToSession(target.localId, { type: "status", status: "processing" });
            sdk.startQuery(target, text, null, ensureProjectAccessForSession(target));
          } else {
            sdk.pushMessage(target, text);
          }
          sm.broadcastSessionList();
          return Promise.resolve({
            targetSessionId: target.localId,
        ...
        var spawnToolDefs = spawnMcp.getToolDefs(function onSpawn(spawnArgs) {
          var vendor = spawnArgs.vendor || "claude";
          var mode = (vendor === "claude" && spawnArgs.mode === "tui") ? "tui" : "gui";
          var derived = deriveTitleFromPrompt(spawnArgs.initialPrompt);
          var title = (derived || String(spawnArgs.title)).substring(0, 100);

          // --- TUI path: launch claude CLI in an xterm immediately so the
          // initial slash command runs in the background and the user
          // returns to in-progress (or completed) work. The "--" separator
          // is required: claude's --mcp-config flag is variadic and would
          // otherwise swallow the positional prompt as a second config file.
          if (mode === "tui") {
            // Eager-launch the PTY so /jira (or whatever the initial
            // prompt is) starts running in the background — the user
            // sees in-progress work the moment they click into the
            // session. pendingInitialPrompt is ALSO stored as a
            // fallback: if claude exits before writing anything to its
            // jsonl, the next click will re-launch from scratch via the
            // switch_session deferred path rather than trying to
            // `--resume` a conversation that never existed.
            if (!tm) {
              return Promise.reject(new Error("Terminal manager unavailable; cannot spawn TUI session."));
            }
            var newSess = sm.createSessionRaw({
              vendor: "claude",
              mode: "tui",
              cliSessionId: crypto.randomUUID(),
            });
            newSess.title = title;
            newSess.titleManuallySet = true;
            newSess.pendingInitialPrompt = String(spawnArgs.initialPrompt);
            newSess.pendingPermissionMode = spawnArgs.permissionMode || "plan";
            newSess.pendingEffort = spawnArgs.effort || "high";
            var spawnedSid = newSess.cliSessionId;
            var spawnedLocalId = newSess.localId;
            var mcpCfgPath = tuiMcpForSpawn.buildTuiMcpConfig(slug, opts, spawnedSid);
            var cmd = "claude --session-id " + spawnedSid +
              " -n " + tuiMcpForSpawn.shellQuote(title) +
              " --permission-mode " + tuiMcpForSpawn.shellQuote(newSess.pendingPermissionMode) +
              " --effort " + tuiMcpForSpawn.shellQuote(newSess.pendingEffort);
            if (mcpCfgPath) cmd += " --mcp-config " + tuiMcpForSpawn.shellQuote(mcpCfgPath);
            cmd += " -- " + tuiMcpForSpawn.shellQuote(spawnArgs.initialPrompt);
            cmd += "; exit\n";
            var claudeJsonlPath = (function () {
              try {
                var pPath = require("path");
                var encoded = require("./utils").encodeCwd(cwd);
                return pPath.join(require("./config").REAL_HOME, ".claude", "projects", encoded, spawnedSid + ".jsonl");
              } catch (e) { return null; }
            })();
            var spawnedActivityHook = (typeof sm.makeTuiActivityHook === "function")
              ? sm.makeTuiActivityHook(spawnedLocalId) : undefined;
            // No requesting ws here (MCP-initiated spawn). Resolve the
            // scrollback cap from the new session's owner if it has one,
            // otherwise null yields the single-user/daemon-default (50 KB).
            var spawnedScrollUserId = newSess.ownerId || null;
            var spawnedScrollbackMax = (typeof sm.tuiScrollbackBytesForUser === "function")
              ? sm.tuiScrollbackBytesForUser(spawnedScrollUserId) : undefined;
            var spawnedTerm = tm.create(80, 24, null, null, {
              initialInput: cmd,
              kind: "tui-session",
              title: "claude " + spawnedSid.slice(0, 8),
              scrollbackMax: spawnedScrollbackMax,
              scrollbackUserId: spawnedScrollUserId,
              onData: spawnedActivityHook,
              onExit: function () {
                tuiMcpForSpawn.cleanupTuiMcpConfig(mcpCfgPath);
                var ds = sm.sessions.get(spawnedLocalId);
                if (!ds) return;
                ds.terminalId = null;
                // If claude actually persisted a conversation to its own
                // jsonl store, the initial prompt is no longer needed —
                // future opens should `--resume` against the saved
                // conversation. If the file doesn't exist, claude exited
                // before writing anything (auth failure, immediate crash,
                // etc.) — keep the pending fields so the next open
                // retries from scratch instead of failing on --resume.
                var hasJsonl = false;
                if (claudeJsonlPath) {
                  try { hasJsonl = require("fs").statSync(claudeJsonlPath).isFile(); } catch (e) {}
                }
                if (hasJsonl) {
                  delete ds.pendingInitialPrompt;
                  delete ds.pendingPermissionMode;
                  delete ds.pendingEffort;
                }
                try { sm.saveSessionFile(ds); } catch (e) {}
                try { sm.broadcastSessionList(); } catch (e) {}
              },
            });
            if (spawnedTerm) newSess.terminalId = spawnedTerm.id;
            sm.saveSessionFile(newSess);
            // Tail the per-session jsonl so claude's auto-titles and
            // response previews surface in Clay even though the actual
            // chat lives in the xterm.
            if (typeof sm.startTuiTitleWatcher === "function") {
              try { sm.startTuiTitleWatcher(newSess); } catch (e) {}
            }
            sm.broadcastSessionList();
            return Promise.resolve({
              sessionId: newSess.localId,
              cliSessionId: newSess.cliSessionId,
              title: newSess.title,
              mode: "tui",
              vendor: "claude",
            });
          ...
          // --- GUI path: SDK-driven query (existing flow) ---
          var newSess = sm.createSessionRaw({ vendor: vendor });
          newSess.title = title;
          newSess.titleManuallySet = true;
          var userMsg = { type: "user_message", text: spawnArgs.initialPrompt, _ts: Date.now() };
          newSess.history.push(userMsg);
          sm.saveSessionFile(newSess);
          sm.broadcastSessionList();
          sendToSession(newSess.localId, userMsg);
          newSess.isProcessing = true;
          onProcessingChanged();
          sendToSession(newSess.localId, { type: "status", status: "processing" });
          sdk.startQuery(newSess, spawnArgs.initialPrompt, null, ensureProjectAccessForSession(newSess));
          return Promise.resolve({
            sessionId: newSess.localId,
            cliSessionId: newSess.cliSessionId || null,
            title: newSess.title,
            mode: "gui",
            vendor: vendor,
          });
        }, onMarkDone, onRename, onSendMessage, onRequestAttention);
        var spawnMcpConfig = adapter.createToolServer({ name: "clay-sessions", version: "1.0.0", tools: spawnToolDefs });
        if (spawnMcpConfig) servers[spawnMcpConfig.name || "clay-sessions"] = spawnMcpConfig;
      } catch (e) {
        console.error("[project] Failed to create spawn MCP server:", e.message);
      }
    }
  ...
  function handleMessage(ws, msg) {
    ...
    if (msg.type === "get_vendor_models") {
      (async function() {
        ...
        sendTo(ws, { type: "model_info", model: modelToSend, models: vendorModels, vendor: msg.vendor, defaultModel: (sm.defaultModelByVendor && sm.defaultModelByVendor[msg.vendor]) || "", availableVendors: sm.availableVendors || [], installedVendors: sm.installedVendors || [] });
      })();
      return;
    }
  ...
  function setLabel(newLabel) {
    // Free-form short text rendered in the icon strip when no emoji
    // icon is set. Capped at 6 chars so it fits the narrow column.
    if (typeof newLabel === "string") {
      var trimmed = newLabel.trim();
      label = trimmed ? trimmed.substring(0, 6) : null;
    } else {
      label = null;
    }
  }

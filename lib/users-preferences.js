function attachPreferences(deps) {
  ...
  // --- Per-user TUI scrollback cap (KB) ---
  //
  // How many KB of scrollback a Claude TUI terminal retains before the
  // oldest chunks are trimmed (see terminal-manager.js). The default 50 KB
  // was sized for the bottom-panel shell; TUI sessions are byte-expensive
  // (the ink/yoga UI redraws its whole box per token, ~10-20 KB/visible
  // turn), so users who rely on scrolling back — especially on mobile —
  // want more. Stored in KB; terminal-manager multiplies to bytes.
  //
  // Storage mirrors alwaysRestartInTui above: multi-user writes to the
  // users.json row; single-user falls back to daemon.json. The setter is a
  // trust boundary — it clamps regardless of what the client sends.

  var TUI_SCROLLBACK_DEFAULT_KB = 50;
  var TUI_SCROLLBACK_MIN_KB = 10;
  var TUI_SCROLLBACK_MAX_KB = 1024;

  function normalizeTuiScrollbackKB(kb) {
    var n = Number(kb);
    if (!Number.isFinite(n)) n = TUI_SCROLLBACK_DEFAULT_KB;
    n = Math.round(n);
    if (n < TUI_SCROLLBACK_MIN_KB) n = TUI_SCROLLBACK_MIN_KB;
    if (n > TUI_SCROLLBACK_MAX_KB) n = TUI_SCROLLBACK_MAX_KB;
    return n;
  }

  function loadSingleUserTuiScrollbackKB() {
    try {
      var config = require("./config");
      var cfg = config.loadConfig() || {};
      if (cfg.tuiScrollbackKB === undefined) return TUI_SCROLLBACK_DEFAULT_KB;
      return normalizeTuiScrollbackKB(cfg.tuiScrollbackKB);
    } catch (e) {
      return TUI_SCROLLBACK_DEFAULT_KB;
    }
  }

  function saveSingleUserTuiScrollbackKB(kb) {
    try {
      var config = require("./config");
      var cfg = config.loadConfig() || {};
      cfg.tuiScrollbackKB = kb;
      config.saveConfig(cfg);
    } catch (e) {}
  }

  function getTuiScrollbackKB(userId) {
    var data = loadUsers();
    for (var i = 0; i < data.users.length; i++) {
      if (data.users[i].id === userId) {
        if (data.users[i].tuiScrollbackKB === undefined) return TUI_SCROLLBACK_DEFAULT_KB;
        return normalizeTuiScrollbackKB(data.users[i].tuiScrollbackKB);
      }
    }
    return loadSingleUserTuiScrollbackKB();
  }

  function setTuiScrollbackKB(userId, kb) {
    var normalized = normalizeTuiScrollbackKB(kb);
    var data = loadUsers();
    for (var i = 0; i < data.users.length; i++) {
      if (data.users[i].id === userId) {
        data.users[i].tuiScrollbackKB = normalized;
        saveUsers(data);
        return { ok: true, tuiScrollbackKB: normalized };
      }
    }
    // Single-user fallback
    saveSingleUserTuiScrollbackKB(normalized);
    return { ok: true, tuiScrollbackKB: normalized };
  }
  ...
  return {
    ...
    getTuiScrollbackKB: getTuiScrollbackKB,
    setTuiScrollbackKB: setTuiScrollbackKB,

...
var SCROLLBACK_MAX = 50 * 1024; // default 50 KB per terminal (shell tabs)

// Drop the oldest scrollback chunks until the buffer is within the
// session's cap. Pure helper over a session-like object
// ({ scrollback, scrollbackSize, scrollbackMax }) so it can be unit-tested
// without a live PTY and reused for live cap changes. Always keeps at least
// one chunk so a single oversized chunk doesn't empty the buffer.
function trimScrollback(session) {
  while (session.scrollbackSize > session.scrollbackMax && session.scrollback.length > 1) {
...
function createTerminalManager(opts) {
   ...
   *   - scrollbackMax: per-terminal scrollback cap in bytes. Defaults to
   *     SCROLLBACK_MAX. TUI sessions pass the owning user's configured cap so
   *     scrollback retention is user-tunable; shell tabs omit it.
   *   - scrollbackUserId: id of the user this terminal's scrollback cap
   *     belongs to (or null). Used by setScrollbackMaxForUser to apply a
   *     live cap change to a user's running TUI terminals.
   */
  function create(cols, rows, osUserInfo, ownerWs, opts) {
    ...
    var session = {
      ...
      scrollback: [],
      scrollbackSize: 0,
      scrollbackMax: (opts && opts.scrollbackMax) || SCROLLBACK_MAX,
      scrollbackUserId: (opts && opts.scrollbackUserId) || null,
      totalBytesWritten: 0,
      cols: cols || 80,
      rows: rows || 24,
  ...
  function attach(id, ws) {
    ...
    // Transfer PTY ownership to the most recently attached client. The
    // resize() owner check still prevents SIGWINCH flood from concurrent
    // observers — but when a user switches between devices/tabs (closes a
    // desktop tab, opens the PWA on mobile, later goes back to desktop),
    // each re-attach should claim ownership so the PTY's cols match the
    // currently-active screen. Without this, the previous owner's
    // dimensions stick even after the user has moved on.
    session.ownerWs = ws;
  ...
  function resize(id, cols, rows, sourceWs) {
    ...
    //
    // EXCEPTION: if the recorded owner WS is gone (disconnected / closed), the
    // session has no live owner. Without this transfer, a TUI re-attached from
    // a different device or tab can never correct the PTY's cols, so claude
    // renders for the *previous* owner's screen size and the new viewer sees
    // heavily-wrapped output. When the owner is dead, the first non-owner to
    // resize becomes the new owner.
    if (session.ownerWs && sourceWs && sourceWs !== session.ownerWs) {
      var ownerAlive = session.ownerWs.readyState === 1;
      if (ownerAlive) return;
      session.ownerWs = sourceWs;
    }
    if (cols > 0 && rows > 0) {
      try {
  ...
  // Apply a new scrollback cap (bytes) to this user's live TUI terminals.
  // Lowering the cap trims the buffer down immediately; raising it just lets
  // the buffer grow further from now on (already-trimmed bytes are gone).
  // Used by the set_tui_scrollback handler so a pref change takes effect on
  // running sessions in this project without a restart. Only affects
  // tui-session terminals tagged with the given user; shell tabs are
  // untouched.
  function setScrollbackMaxForUser(userId, bytes) {
    if (!(bytes > 0)) return;
    for (var session of terminals.values()) {
      if (session.kind === "tui-session" && session.scrollbackUserId === userId) {
        session.scrollbackMax = bytes;
        trimScrollback(session);
      }
    }
  }
  ...
  function destroyAll() {
    for (var session of terminals.values()) {
      // Shutdown teardown: pty.kill triggers pty.onExit asynchronously,
      // and that handler would normally invoke onExitHook (which for TUI
      // sessions clears terminalId AND, on the new_session path, used to
      // delete the Clay session record). On a graceful daemon stop we
      // don't want any of that to fire — sessions should survive the
      // restart untouched. Null the hook so onExit becomes a no-op for
      // cleanup-on-shutdown.
  ...
  return {
    ...
    close: close,
    rename: rename,
    setScrollbackMaxForUser: setScrollbackMaxForUser,
    list: list,

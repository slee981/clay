...
function createServer(opts) {
  ...
  var onSetProjectLabel = opts.onSetProjectLabel || null;
  ...
  var onSetShowCodex = opts.onSetShowCodex || null;
  ...
  (function installTuiHook() {
    try {
      ...
      // custom additions from claudeUserAllowList.
  ...
  function addProject(cwd, slug, title, icon, projectOwnerId, worktreeMeta, extraOpts) {
    ...
    var ctx = createProjectContext({
      ...
      onSetProjectLabel: onSetProjectLabel,
      onProjectOwnerChanged: onProjectOwnerChanged,
  ...
  function setProjectLabel(slug, label) {
    var ctx = projects.get(slug);
    if (!ctx) return false;
    ctx.setLabel(label);
    return true;

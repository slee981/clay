...
function createSessionManager(opts) {
  ...
  function saveSessionFile(session) {
    ...
    try {
      ...
      // Persist mode explicitly so born mode is sticky across daemon restarts.
      // Legacy files written before this field was explicit reload as gui via
      // the matching fallback in the loader.
      metaObj.mode = session.mode === "tui" ? "tui" : "gui";
      ...
      if (session.pendingPermissionMode) metaObj.pendingPermissionMode = session.pendingPermissionMode;
      if (session.pendingEffort) metaObj.pendingEffort = session.pendingEffort;
      // Structured "done" state. Persist when true OR when explicitly
      // false (so the legacy "done - " migration can record that this
      // session has already been considered and shouldn't be re-migrated).
      if (typeof session.done === "boolean") metaObj.done = !!session.done;
      // Free-form session "kind" tag. Sidebar maps known kinds to icons
      // (e.g. "sprint" -> git-branch glyph). Persist as-is.
      if (typeof session.kind === "string" && session.kind) metaObj.kind = session.kind;
      // "Needs user input" flag set by an agent via request_user_attention
      // when it's blocked at an approval gate. Sidebar highlights the
      // session until the user opens it (auto-clear in switch_session).
      if (session.needsAttention) metaObj.needsAttention = session.needsAttention;
      // User-chosen sidebar accent color (right-click → Set color). One of
      // the named swatches enforced server-side; null/absent means default.
      if (typeof session.color === "string" && session.color) metaObj.color = session.color;
  ...
  function loadSessions() {
    ...
    for (var i = 0; i < loaded.length; i++) {
      ...
      if (m.pendingPermissionMode) session.pendingPermissionMode = m.pendingPermissionMode;
      if (m.pendingEffort) session.pendingEffort = m.pendingEffort;
      // Done flag. Falls back to migrating the legacy "done - " title
      // prefix when the field hasn't been recorded yet.
      if (typeof m.done === "boolean") {
        session.done = m.done;
      } else if (typeof m.title === "string" && m.title.indexOf("done - ") === 0) {
        session.done = true;
        session.title = m.title.substring("done - ".length);
        session.titleManuallySet = true;
      } else {
        session.done = false;
      }
      // Optional kind tag (sprint dispatcher, etc.). Free string; sidebar
      // decides which icon (if any) to render based on the value.
      if (typeof m.kind === "string" && m.kind) session.kind = m.kind;
      if (m.needsAttention && typeof m.needsAttention === "object") session.needsAttention = m.needsAttention;
      if (typeof m.color === "string" && m.color) session.color = m.color;
  ...
  return {
    ...
    findSessionByTerminalId: function (terminalId) {
      if (typeof terminalId !== "number") return null;
      var found = null;
      sessions.forEach(function (s) {
        if (s.terminalId === terminalId || s.runtimeTerminalId === terminalId) found = s;
      });
      return found;
    },
    // Bump lastActivity. Persistence/broadcast is throttled per-session
    // (default 60s) so high-frequency callers (PTY input) don't write the
    // meta jsonl on every keystroke. Pass {force: true} to persist
    // immediately (e.g. on user session open).
    bumpSessionActivity: function (localId, optsOrUndef) {
      var s = sessions.get(localId);
      if (!s) return;
      var force = !!(optsOrUndef && optsOrUndef.force);
      var now = Date.now();
      s.lastActivity = now;
      if (force || !s._lastActivityPersistedAt || (now - s._lastActivityPersistedAt) > 60000) {
        s._lastActivityPersistedAt = now;
        saveSessionFile(s);
        broadcastSessionList();
      }
    },

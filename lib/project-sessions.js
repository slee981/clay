  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "switch_session") {
      if (msg.id && sm.sessions.has(msg.id)) {
        ...
        // Opening a session is a fresh activity signal — bump lastActivity
        // so the sidebar moves it to "today" even if no further input
        // arrives. TUI sessions don't otherwise route their typing through
        // Clay's history (claude writes the jsonl directly), so this is
        // the primary mechanism to keep their timestamp current.
        try { sm.bumpSessionActivity(msg.id, { force: true }); } catch (e) {}

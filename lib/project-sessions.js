...
function attachSessions(ctx) {
  ...
  function spawnRuntimeTuiPty(session, ws) {
    ...
    var mcpConfigPath = tuiMcp.buildTuiMcpConfig(slug, opts);
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "new_session") {
      ...
      if (requestedMode === "tui") {
        ...
        if (tm) {
          ...
          var tuiMcpPath = tuiMcp.buildTuiMcpConfig(slug, opts);
    ...
    if (msg.type === "switch_session") {
      if (msg.id && sm.sessions.has(msg.id)) {
        ...
        // Deferred launch for spawn_session TUI sessions: spawn_mcp creates
        // the session record only and defers `claude` until first open so
        // the PTY has an attached client and stays interactive. Launch now.
        if (xmTarget && xmTarget.mode === "tui" && xmTarget.pendingInitialPrompt && !xmTarget.terminalId && tm) {
          var dlPrompt = xmTarget.pendingInitialPrompt;
          ...
          var dlMcpPath = tuiMcp.buildTuiMcpConfig(slug, opts);
          var dlCmd = "claude --session-id " + dlSid +
          ...
          var dlTerm = tm.create(80, 24, getOsUserInfoForWs(ws), ws, {
            ...
            onExit: function () {
              ...
              // Keep the session record: the conversation is persisted by
              // the claude CLI under ~/.claude/projects/<cwd>/<uuid>.jsonl
              // and the user can reopen it (resume via runtime PTY).

...
var { buildConfigDefaults } = require("./config-defaults");
...
function attachSessions(ctx) {
  ...
  // Broadcast the persisted server/project defaults to every client on this
  // connection so their Settings default bars reflect what is saved on disk,
  // independent of the live session's config_state. Called on each default
  // setter so a change in one browser propagates to the others.
  function sendConfigDefaults() {
    send(buildConfigDefaults(opts, slug, ctx.debug ? function (m) { console.log(m); } : null));
  }
  ...
  sm.tuiScrollbackBytesForUser = tuiScrollbackBytesForUser;

  // Resolve a user's configured TUI scrollback cap (KB pref) to a byte count
  // for terminal-manager. Passed into tm.create opts at TUI-session create
  // sites so each terminal's buffer honors the owning user's setting. A null
  // userId yields the single-user/daemon-default value (50 KB).
  function tuiScrollbackBytesForUser(userId) {
    if (usersModule && typeof usersModule.getTuiScrollbackKB === "function") {
      return usersModule.getTuiScrollbackKB(userId) * 1024;
    }
    return 50 * 1024;
  }
  ...
  function spawnRuntimeTuiPty(session, ws) {
    ...
    cmd += "; exit\n";
    var _scrollUserId = (ws && ws._clayUser) ? ws._clayUser.id : null;
    var term = tm.create(80, 24, getOsUserInfoForWs(ws), ws, {
      initialInput: cmd,
      kind: "tui-session",
      title: "claude (resume) " + sid.slice(0, 8),
      scrollbackMax: tuiScrollbackBytesForUser(_scrollUserId),
      scrollbackUserId: _scrollUserId,
      onExit: function () {
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "new_session") {
      ...
      if (requestedMode === "tui") {
        ...
        if (tm) {
          ...
          tuiCmd += "; exit\n";
          var tuiScrollUserId = (ws && ws._clayUser) ? ws._clayUser.id : null;
          var tuiTerm = tm.create(80, 24, getOsUserInfoForWs(ws), ws, {
            initialInput: tuiCmd,
            kind: "tui-session",
            title: "claude " + tuiSid.slice(0, 8),
            scrollbackMax: tuiScrollbackBytesForUser(tuiScrollUserId),
            scrollbackUserId: tuiScrollUserId,
    ...
    if (msg.type === "set_session_color") {
      if (typeof msg.sessionId === "number") {
        var colorTarget = sm.sessions.get(msg.sessionId);
        if (colorTarget) {
          var allowedColors = ["red", "orange", "yellow", "green", "blue", "purple", "pink", "gray"];
          var nextColor = (msg.color == null || msg.color === "") ? null : String(msg.color).toLowerCase();
          if (nextColor !== null && allowedColors.indexOf(nextColor) === -1) {
            return true;
          }
          colorTarget.color = nextColor;
          sm.saveSessionFile(colorTarget);
          sm.broadcastSessionList();
        }
      }
      return true;
    ...
    if (msg.type === "switch_session") {
      if (msg.id && sm.sessions.has(msg.id)) {
        ...
        if (xmTarget && xmTarget.mode === "tui" && !xmTarget.terminalId && tm && xmTarget.cliSessionId) {
          ...
          dlCmd += "; exit\n";
          var dlScrollUserId = (ws && ws._clayUser) ? ws._clayUser.id : null;
          var dlTerm = tm.create(80, 24, getOsUserInfoForWs(ws), ws, {
            initialInput: dlCmd,
            kind: "tui-session",
            title: "claude " + dlSid.slice(0, 8),
            scrollbackMax: tuiScrollbackBytesForUser(dlScrollUserId),
            scrollbackUserId: dlScrollUserId,
    ...
    if (msg.type === "set_vendor" && msg.vendor) {
      ...
      if (msg.vendor) {
        ...
        sendTo(ws, {
          ...
          models: vendorModels,
          vendor: msg.vendor,
          defaultModel: (sm.defaultModelByVendor && sm.defaultModelByVendor[msg.vendor]) || "",
    ...
    if (msg.type === "set_effort" && msg.effort) {
      // In-session effort change: deliberately does NOT touch
      // sm._savedDefaultEffort or call sendConfigDefaults() — it is ephemeral
      // and must not leak into new sessions or the Settings default bar. Only
      // the set_*_default_effort handlers below persist and broadcast.
    ...
    if (msg.type === "set_server_default_effort" && msg.effort) {
      ...
      // Keep the saved default current so new sessions on this connection honor
      // it without a restart. The in-session control (set_effort) deliberately
      // does NOT touch _savedDefaultEffort.
      sm._savedDefaultEffort = msg.effort;
      sm.currentEffort = msg.effort;
      send({ type: "config_state", model: sm.currentModel || "", mode: sm.currentPermissionMode || "default", effort: sm.currentEffort, betas: sm.currentBetas || [], thinking: sm.currentThinking || "adaptive", thinkingBudget: sm.currentThinkingBudget || 10000 });
      sendConfigDefaults();
      return true;
    ...
    if (msg.type === "set_tui_scrollback") {
      // Per-user preference: how many KB of scrollback a Claude TUI terminal
      // retains. The setter clamps server-side, so a bad client value can't
      // escape the [10, 1024] KB range. On success, apply the new cap to this
      // user's live TUI terminals in this project (forward-only — lowering
      // trims now, raising retains more from here on); other projects pick it
      // up on next session open. In single-user mode userId is null and the
      // setter falls through to daemon.json storage.
      var tsbUserId = ws._clayUser ? ws._clayUser.id : null;
      var tsbResult = usersModule.setTuiScrollbackKB(tsbUserId, msg.kb);
      if (tsbResult && tsbResult.ok) {
        if (tm && typeof tm.setScrollbackMaxForUser === "function") {
          tm.setScrollbackMaxForUser(tsbUserId, tsbResult.tuiScrollbackKB * 1024);
        }
        sendTo(ws, { type: "set_tui_scrollback_result", ok: true, tuiScrollbackKB: tsbResult.tuiScrollbackKB });
        sendTo(ws, { type: "tui_scrollback_changed", tuiScrollbackKB: tsbResult.tuiScrollbackKB });
      } else {
        sendTo(ws, { type: "set_tui_scrollback_result", ok: false, error: (tsbResult && tsbResult.error) || "unknown" });
      }
      return true;
    }

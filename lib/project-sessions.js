...
function attachSessions(ctx) {
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "list_cli_sessions") {
      ...
      // Union of two sources, because the SDK's listSessions returns a
      // subset of what's on disk for some projects (sessions claude --resume
      // happily finds via direct .jsonl scan don't always come back from
      // the SDK). We always scan the filesystem and merge any SDK metadata
      // on top, so Import CLI shows everything claude itself can resume.
      var cliSessionsMod = require("./cli-sessions");
      var fsScan = cliSessionsMod.listCliSessions(cwd).catch(function () { return []; });
      ...
      Promise.all([fsScan, sdkScan]).then(function (pair) {
        ...
        var merged = {};
        // Seed from FS scan (authoritative for existence).
        for (var fi = 0; fi < fsList.length; fi++) {
          if (fsList[fi] && fsList[fi].sessionId) merged[fsList[fi].sessionId] = fsList[fi];
        }
        // Add any SDK-only entries we haven't seen (shouldn't normally happen).

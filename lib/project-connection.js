...
function attachConnection(ctx) {
  ...
  function handleConnection(ws, wsUser, handleMessage, handleDisconnection) {
    ...
    if (sm.currentModel) {
      sendTo(ws, { type: "model_info", model: sm.currentModel, models: initialModels, vendor: initialVendor, defaultModel: (sm.defaultModelByVendor && sm.defaultModelByVendor[initialVendor]) || "", availableVendors: sm.availableVendors || [], installedVendors: sm.installedVendors || [] });
    }
    ...
    // Persisted server/project defaults, separate from the live config_state
    // above, so the Settings default bars reflect what is saved on disk.
    sendTo(ws, buildConfigDefaults(opts, slug, debug ? function (m) { console.log(m); } : null));
    ...
    // Initial per-user preference: how to render Claude sessions. In
    // single-user mode wsUser is null and the getter falls through to the
    // daemon.json fallback.
    ...
    if (usersModule && typeof usersModule.getAlwaysRestartInTui === "function") {
      ...
      var _artVal = usersModule.getAlwaysRestartInTui(_artUid);
      sendTo(ws, { type: "always_restart_in_tui_changed", alwaysRestartInTui: !!_artVal });
    }
    // Initial per-user preference: TUI scrollback retention (KB).
    if (usersModule && typeof usersModule.getTuiScrollbackKB === "function") {
      var _tsbUid = (wsUser && wsUser.id) || null;
      var _tsbVal = usersModule.getTuiScrollbackKB(_tsbUid);
      sendTo(ws, { type: "tui_scrollback_changed", tuiScrollbackKB: _tsbVal });
    }

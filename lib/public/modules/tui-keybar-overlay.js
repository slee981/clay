...
// getter for the existing handler, and there is exactly ONE slot. There are
// three writers of it: session-tui-view.js's createXterm (Shift+Enter ->
// newline, the desktop owner), this overlay's installXtermKeyHandler (keybar
// active, coarse pointer), and installPassthroughKeyHandler (keybar hidden /
// fine pointer). Whichever runs last wins the slot.
//
// INSTALL-ORDER INVARIANT: every writer of the slot MUST call
// maybeSendTuiNewline(ev, sendSeq) first and return false when it returns
// true, so Shift+Enter -> newline holds no matter which writer claimed the
// slot last (e.g. the passthrough installed on a coarse->fine flip must not
// silently drop it). Any future fourth writer must do the same.
//
// CLAUDE.md compliance: `var` only, no arrow functions, no `_ctx` capture.

import { getWs } from "./ws-ref.js";
import { showToast } from "./utils.js";
import { createKeybar, consumeCtrlKey, maybeSendTuiNewline } from "./term-keybar.js";
...
function installXtermKeyHandler() {
  ...
  xt.attachCustomKeyEventHandler(function (ev) {
    // Shift+Enter -> literal newline. Checked first and independent of the
    // keybar so the behavior survives whichever handler owns the single slot
    // (see installPassthroughKeyHandler and the slot-ownership note above).
    if (maybeSendTuiNewline(ev, sendSeq)) return false;
...
function installPassthroughKeyHandler() {
  ...
  if (!xt || typeof xt.attachCustomKeyEventHandler !== "function") return;
  try {
    // Not a bare passthrough: the Shift+Enter -> newline check must survive
    // even after the keybar unmounts (coarse->fine pointer flip), otherwise
    // this noop would clobber the createXterm handler and silently drop it.
    xt.attachCustomKeyEventHandler(function (ev) {
      if (maybeSendTuiNewline(ev, sendSeq)) return false;
      return true;
    });
...
function mountOverlay() {
  ...
  // Del Word (Ctrl+W) and Clear (Ctrl+U) are TUI-specific utility buttons,
  // placed at the head of the last button group (just before Alt).
  // utility:true routes them through the data-keybar-utility delegate below,
  // bypassing the keybar's sticky Ctrl/Alt logic (an armed Alt can't turn
  // \x17 into \x1b\x17, an armed Ctrl can't silently consume them).
  keybar = createKeybar(overlayEl, {
    insertBeforeKey: "alt",
    insertButtons: [
      { key: "delword", label: "Del Word", classes: ["term-key"], utility: true },
      { key: "clear", label: "Clear", classes: ["term-key"], utility: true },
    ],
  });
  ...
  overlayEl.addEventListener("click", function (e) {
    ...
    // Ctrl+W: delete the word left of the cursor (repeatable, bash-style).
    if (key === "delword") { sendSeq("\x17"); return; }
    // Clear the whole input line in one tap, regardless of cursor position:
    // Ctrl+E (jump to end) then Ctrl+U (kill to start). Ctrl+U alone is
    // readline kill-to-start and would leave text right of the cursor; the
    // leading Ctrl+E makes it whole-line in both TUI interpretations. Ctrl+E
    // is a harmless no-op on an empty line. Non-destructive — does NOT
    // trigger TUI rewind/checkpoint (that is double-Esc, deliberately avoided).
    if (key === "clear") { sendSeq("\x05\x15"); return; }
  });

...
import { isProjectSettingsOpen, handleInstructionsRead, handleInstructionsWrite, handleProjectEnv, handleProjectEnvSaved, handleProjectSharedEnv, handleProjectSharedEnvSaved, handleProjectOwnerChanged, updateSettingsDefaults as updateProjectSettingsDefaults } from './project-settings.js';
import { updateSettingsModels, updateSettingsStats, updateDaemonConfig, handleSetPinResult, handleKeepAwakeChanged, handleAutoContinueChanged, handleRestartResult, handleShutdownResult, handleSharedEnv, handleSharedEnvSaved, handleGlobalClaudeMdRead, handleGlobalClaudeMdWrite, updateSettingsDefaults as updateServerSettingsDefaults } from './server-settings.js';
...
import { getModelEffortLevels, clampEffortToSupported, accumulateUsage, updateUsagePanel, accumulateContext, updateContextPanel, renderCtxPopover, updateStatusPanel } from './app-panels.js';
...
export function processMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case "model_info": {
        ...
        // SDK-reported concrete default model for this vendor (issue #22).
        // Always carried (possibly "") so vendor switches clear a stale default.
        var _sdkDefault = Object.prototype.hasOwnProperty.call(msg, "defaultModel")
          ? (msg.defaultModel || "")
          : store.get('sdkDefaultModel');
        var _miUpdate = { currentModels: msg.models || [], sdkDefaultModel: _sdkDefault };
        ...
        store.set(_miUpdate);
        updateSettingsModels(_modelVal, msg.models || [], _sdkDefault);
        break;
      }
      ...
      case "config_state": {
        ...
        if (_csRead.currentModels.length > 0) {
          ...
          var clampedEffort = clampEffortToSupported(_csRead.currentEffort, levels);
          if (clampedEffort !== _csRead.currentEffort) store.set({ currentEffort: clampedEffort });
        }
        } break;

      case "config_defaults": {
        // Persisted server/project defaults (effort/mode/model), distinct from
        // the live config_state above. The Settings default bars bind to these.
        store.set({
          serverDefaultEffort: msg.serverDefaultEffort || null,
          projectDefaultEffort: msg.projectDefaultEffort || null,
          serverDefaultMode: msg.serverDefaultMode || null,
          projectDefaultMode: msg.projectDefaultMode || null,
          serverDefaultModel: msg.serverDefaultModel || null,
          projectDefaultModel: msg.projectDefaultModel || null,
        });
        // Legacy settings panels don't subscribe to the store; push explicitly.
        // Each stashes the values and re-renders its effort bar if open.
        updateServerSettingsDefaults(msg);
        updateProjectSettingsDefaults(msg);
        break;
      }
      ...
      case "session_switched":
        ...
        // Sessions normally render in their born mode (msg.mode). When the
        // user's alwaysRestartInTui pref is on, the server attaches a runtime
        // PTY to born-GUI Claude sessions on switch and surfaces it via
        // runtimeMode/runtimeTerminalId so the client renders them as TUI
        // without mutating the on-disk session.
        ...
        // Also flip `processing` so the Send/Stop button enters Stop mode
        // when the user switches into a session that's already mid-turn.
        // The store subscriber in app-connection.js drives the button off
        // `processing`, not `sessionIsProcessing` (that flag is for todo
        // compaction only).
        store.set({ activeSessionId: msg.id, cliSessionId: msg.cliSessionId || null, vendorCapabilities: msg.capabilities || {}, sessionIsProcessing: !!msg.isProcessing, processing: !!msg.isProcessing, activeSessionMode: _effectiveMode, activeTerminalId: _effectiveTerminalId, sessionHasHistory: !!msg.hasHistory });
      ...
      case "set_always_restart_in_tui_result":
      case "always_restart_in_tui_changed":
        if (typeof msg.alwaysRestartInTui === "boolean") {
          store.set({ alwaysRestartInTui: msg.alwaysRestartInTui });
          var _artToggle = document.getElementById("us-always-restart-tui");
          if (_artToggle) _artToggle.checked = !!msg.alwaysRestartInTui;
        }
        break;

      case "set_tui_scrollback_result":
      case "tui_scrollback_changed":
        if (typeof msg.tuiScrollbackKB === "number") {
          store.set({ tuiScrollbackKB: msg.tuiScrollbackKB });
          var _tsbSelect = document.getElementById("us-tui-scrollback");
          if (_tsbSelect) _tsbSelect.value = String(msg.tuiScrollbackKB);
        }
        break;

      case "set_show_codex_result":
      case "show_codex_changed":
        store.set({ showCodex: !!msg.showCodex });
        var _scToggle = document.getElementById("us-show-codex");
        if (_scToggle) _scToggle.checked = !!msg.showCodex;
        // Re-render the session list so the Codex new-session button
        // appears/disappears immediately on flip without a refresh.
        try { renderSessionList(null); } catch (e) {}
        break;

...
import { maybeSendTuiNewline } from './term-keybar.js';
...
// Owning project slug for the bound terminal. Terminal ids are assigned
// per-project (terminal-manager.js resets nextId=1 per project), so they
// are NOT globally unique — project A's term 1 and project B's term 1
// collide. Tracking the slug alongside the id lets the "same terminal"
// short-circuit fire only when BOTH the project and the id match, so a
// cross-project TUI->TUI switch with colliding ids still tears down and
// re-attaches instead of keeping the stale xterm. See issue #6.
...
var deferredFitTimer = 0; // setTimeout handle for the post-attach settle fit
...
function syncHostBounds() {
  ...
  //
  // Supersedes Tushar's earlier nav-bar-only fix (399485a) — same problem,
  // but his version didn't know about the keybar overlay added later.
...
function createXterm() {
  ...
  // Send raw bytes to the bound PTY. No-ops (returns false) when the terminal
  // is unbound or the socket is closed; returns true when the frame was
  // queued. Shared by onData and the Shift+Enter handler; both currently
  // ignore the return value (it's exposed for callers that may want it).
  function sendInput(data) {
    if (currentTermId == null) return false;
    var ws = getWs();
  ...
  // Shift+Enter inserts a literal newline instead of submitting. onData can't
  // see the Shift modifier (it relays already-processed data — both Enter and
  // Shift+Enter arrive as \r), so we intercept at the raw key-event layer.
  // maybeSendTuiNewline returns true for every phase of the chord (keydown,
  // keypress, keyup); returning false here suppresses xterm so it never emits
  // the submitting \r (the keypress phase is the one that would otherwise
  // submit). On desktop this is the only writer of the single
  // attachCustomKeyEventHandler slot; on touch the keybar overlay owns it and
  // runs the same maybeSendTuiNewline check first.
  term.attachCustomKeyEventHandler(function (ev) {
    if (maybeSendTuiNewline(ev, sendInput)) return false;
    return true;
  });
  return term;
}
...
export function attachTuiView(terminalId) {
  ...
  // Re-attaching to the same terminal in the same project: just refit and
  // refocus. The slug guard prevents a cross-project switch with a
  // colliding terminal id from short-circuiting onto the stale xterm.
  if (currentTermId === terminalId && currentSlug === slug && xterm) {
  ...
  // Switching to a different TUI terminal (different id, or same id in a
  // different project): tear down the old one cleanly.
  if (currentTermId != null && (currentTermId !== terminalId || currentSlug !== slug)) {
    detachTuiView();
  }
  ...
  // attach completes).
  deferredFitTimer = setTimeout(function () { deferredFitTimer = 0; fitNow(); }, 50);

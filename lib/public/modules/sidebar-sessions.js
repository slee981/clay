...
function showSessionCtxMenu(anchorBtn, sessionId, title, cliSid, sessionData) {
  ...
  colorRow.className = "session-ctx-color-row";
  var colorLabel = document.createElement("div");
  colorLabel.className = "session-ctx-color-label";
  colorLabel.textContent = "Color";
  colorRow.appendChild(colorLabel);
  var colorSwatches = document.createElement("div");
  colorSwatches.className = "session-ctx-color-swatches";
  var swatchNames = ["red", "orange", "yellow", "green", "blue", "purple", "pink", "gray"];
  var currentColor = (sessionData && sessionData.color) || null;
  swatchNames.forEach(function (c) {
    var sw = document.createElement("button");
    sw.className = "session-ctx-swatch session-ctx-swatch-" + c + (currentColor === c ? " is-selected" : "");
    sw.title = c.charAt(0).toUpperCase() + c.slice(1);
    sw.setAttribute("aria-label", "Set color " + c);
  ...
  clearSw.className = "session-ctx-swatch session-ctx-swatch-clear" + (!currentColor ? " is-selected" : "");
  clearSw.title = "Clear color";
  clearSw.setAttribute("aria-label", "Clear color");
  clearSw.innerHTML = iconHtml("x");
  clearSw.addEventListener("click", function (e) {
    e.stopPropagation();
    closeSessionCtxMenu();
    if (getWs() && store.get("connected")) {
      getWs().send(JSON.stringify({
  ...
  colorSwatches.appendChild(clearSw);
...
function renderSessionItem(s) {
  var el = document.createElement("div");
  var isMatch = searchMatchIds !== null && searchMatchIds.has(s.id);
  el.className = "session-item" + (s.active ? " active" : "") + (isMatch ? " search-match" : "") + (s.needsAttention ? " needs-attention" : "") + (s.color ? " session-color-" + s.color : "");
  el.dataset.sessionId = s.id;

  var textSpan = document.createElement("span");
  textSpan.className = "session-item-text";
  var textHtml = "";
  // Fixed-width leading status slot so the title text never shifts when
  // the processing dot or attention bell toggle on/off. Slot is always
  // present; one of the two icons may occupy it (bell > dot priority).
  // If neither is active the slot stays empty but still reserves width.
  var slotInner = "";
  if (s.needsAttention) {
    var attReason = (s.needsAttention.reason || "needs your input");
    slotInner = '<span class="session-attention-icon" title="' + escapeHtml(attReason) + '">' + iconHtml("bell-ring") + '</span>';
  } else if (s.isProcessing) {
    slotInner = '<span class="session-processing"></span>';
  }
  textHtml += '<span class="session-status-slot">' + slotInner + '</span>';

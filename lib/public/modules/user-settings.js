...
export function initUserSettings(appCtx) {
  ...
  // TUI scrollback size select: how many KB of scrollback a Claude TUI
  // terminal retains. The server persists the pref per user and clamps the
  // value; the `tui_scrollback_changed` echo keeps this client's store and
  // select in sync (other tabs/devices pick it up on their next connect).
  var tuiScrollbackSelect = document.getElementById('us-tui-scrollback');
  if (tuiScrollbackSelect) {
    var tsbStored = store.get('tuiScrollbackKB');
    tuiScrollbackSelect.value = String(tsbStored != null ? tsbStored : 50);
    tuiScrollbackSelect.addEventListener('change', function () {
      var kb = parseInt(this.value, 10);
      var ws = getWs();
      if (ws && ws.readyState === 1) {
        ws.send(JSON.stringify({ type: 'set_tui_scrollback', kb: kb }));
        showToast('TUI scrollback set to ' + (kb >= 1000 ? (kb / 1000) + ' MB' : kb + ' KB'));
      }
    });
  }

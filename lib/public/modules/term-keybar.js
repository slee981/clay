...
// Render one button definition into toolbarEl. Handles both the spacer
// branch and the <button> branch so leading buttons (opts.leadingButtons)
// and DEFAULT_BUTTONS render through the exact same contract — crucially
// preserving the data-keybar-utility attribute the overlay delegate matches on.
function buildButtonEl(toolbarEl, def) {
  if (def.spacer) {
...
// Render DEFAULT_BUTTONS, optionally splicing host-provided buttons in
// before the default whose key === insertBeforeKey (so the host can place
// its affordances mid-bar, e.g. just before Alt, rather than only at an end).
function buildDefaultButtons(toolbarEl, insertBeforeKey, insertButtons) {
  for (var i = 0; i < DEFAULT_BUTTONS.length; i++) {
    var def = DEFAULT_BUTTONS[i];
    if (insertButtons && insertButtons.length && def.key === insertBeforeKey) {
      for (var k = 0; k < insertButtons.length; k++) {
        buildButtonEl(toolbarEl, insertButtons[k]);
      }
    }
    buildButtonEl(toolbarEl, def);
  }
}
...
export function createKeybar(toolbarEl, opts) {
  ...
  // The host can splice its own buttons into the default row via
  // opts.insertButtons (array of the same {key,label,classes,utility?} shape),
  // positioned just before the default button named by opts.insertBeforeKey.
  if (opts.buildDefaultButtons !== false) {
    buildDefaultButtons(toolbarEl, opts.insertBeforeKey, opts.insertButtons);
  }

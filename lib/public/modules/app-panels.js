...
import { modelLabelWithDefault } from './settings-defaults.js';
...
function rebuildModelList() {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    // Show the resolved model name with " (default)" next to the SDK
    // auto-default option (issue #22), not the raw "Default (recommended)".
    var label = modelLabelWithDefault(value, s.sdkDefaultModel, list) ||
      (typeof item === "string" ? item : (item.displayName || value));
...
export function initPanels() {
  ...
  store.subscribe(function (state, prev) {
    ...
    if (state.currentModel !== prev.currentModel ||
        state.sdkDefaultModel !== prev.sdkDefaultModel ||
...
export function updateConfigChip() {
  ...
  configChipLabel.textContent = modelLabelWithDefault(s.currentModel, s.sdkDefaultModel, s.currentModels);
  rebuildModelList();
  rebuildModeList();
  rebuildEffortBar();
...
// Canonical effort ordering spanning every vendor, used to clamp an effort
// level the active model doesn't support to the nearest one that it does.
var EFFORT_ORDER = ["minimal", "low", "medium", "high", "xhigh", "max"];

// Given a desired effort and the model's supported levels, return the desired
// level if supported, otherwise the nearest supported level by canonical index
// (ties break toward the higher level to preserve a deliberately-high default).
// Never silently collapses to "medium" unless the desired level is unknown and
// "medium" happens to be the only sensible fallback.
export function clampEffortToSupported(effort, levels) {
  if (!effort || !levels || levels.length === 0) return effort;
  for (var i = 0; i < levels.length; i++) {
    if (levels[i] === effort) return effort;
  }
  var targetIdx = EFFORT_ORDER.indexOf(effort);
  if (targetIdx === -1) {
  ...
  for (var j = 0; j < levels.length; j++) {
    var idx = EFFORT_ORDER.indexOf(levels[j]);
    if (idx === -1) continue;
    var dist = Math.abs(idx - targetIdx);

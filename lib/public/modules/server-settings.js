...
var ctx = null;
// Persisted defaults from the config_defaults carrier, stashed module-locally
// (these legacy panels don't subscribe to the store). The effort bar binds to
// ssDefaults.effort so it shows the SAVED server default, not the live session.
var ssDefaults = { effort: null, mode: null, model: null };
var settingsEl = null;
...
function ssDefaultsOpts() {
  return {
    ...
    // Model picker highlight reflects the persisted SERVER default, not the
    // live session — mirrors the #19 effort-bar binding (issue #22).
    currentModel: ssDefaults.model || ctx.currentModel || ctx._currentModelValue || "",
    // Resolves each option's label to its model name + "(default)" marker.
    sdkDefaultModel: ctx.sdkDefaultModel || "",
    currentMode: ctx.currentMode || "default",
    // Effort bar reflects the persisted SERVER default, not the live session.
    currentEffort: ssDefaults.effort || "medium",
...
export function updateSettingsModels(current, models, sdkDefault) {
  if (!ctx) return;
  ctx.currentModels = models;
  ctx._currentModelValue = current;
  // SDK-reported concrete default model id for the active vendor (issue #22).
  // Carried alongside the model_info payload; "" for vendors with no default.
  if (typeof sdkDefault !== "undefined") ctx.sdkDefaultModel = sdkDefault;
  if (isSettingsOpen()) {
    updateModelList();
    updateModeList();
    updateEffortBar();
...
// Stash the persisted server/project defaults from the config_defaults carrier
// and re-render the effort bar so it shows the SAVED server default rather than
// the live session's effort. Always stashes (so a later panel open sees the
// value); re-renders only when the panel is already open.
export function updateSettingsDefaults(defaults) {
  if (!ctx || !defaults) return;
  ssDefaults = {
    effort: defaults.serverDefaultEffort || null,
    mode: defaults.serverDefaultMode || null,
    model: defaults.serverDefaultModel || null,
  };
  if (isSettingsOpen()) {
    updateEffortBar();
    // Persisted default model drives both the picker highlight and the
    // resolved-model indicator (issue #22).
    updateModelList();
  }
}

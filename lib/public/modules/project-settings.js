...
// (this legacy panel doesn't subscribe to the store). The effort bar binds to
// the project default, falling back to the server default, so it shows the
// SAVED value rather than the live session's effort.
var psDefaults = { projectEffort: null, serverEffort: null };
...
function psDefaultsOpts() {
  return {
    ...
    // (issue #22). Shared appCtx, populated by updateSettingsModels.
    sdkDefaultModel: ctx.sdkDefaultModel || "",
    currentMode: ctx.currentMode || "default",
    // Effort bar reflects the persisted PROJECT default, then the server
    // default, not the live session.
    currentEffort: psDefaults.projectEffort || psDefaults.serverEffort || "medium",
...
// and re-render the effort bar so it shows the SAVED project default (falling
// back to the server default) rather than the live session's effort. Always
// stashes; re-renders only when the panel is open.
export function updateSettingsDefaults(defaults) {
  if (!defaults) return;
  psDefaults = {
    projectEffort: defaults.projectDefaultEffort || null,
    serverEffort: defaults.serverDefaultEffort || null,
  };
  if (isProjectSettingsOpen()) renderEffortBar("ps", psDefaultsOpts());
}

...
function createSDKBridge(opts) {
  ...
  function sendModelInfoForVendor(vendor, model) {
    // Resolve the vendor before the defaultModel lookup so the vendor:null
    // path (setModel with no active query) still carries the SDK default.
    ...
    send({
      ...
      defaultModel: (sm.defaultModelByVendor && sm.defaultModelByVendor[resolvedVendor]) || "",
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    var queryOpts = {
      cwd: cwd,
      model: queryModel,
      // New sessions resolve effort as: resumed session's own effort ->
      // persisted project/server default -> "medium". Deliberately NOT
      // sm.currentEffort, which the in-session control mutates: a mid-connection
      // effort tweak must not leak into newly created sessions (issue #19).
      effort: ls.effort || sm._savedDefaultEffort || "medium",
  ...
  async function warmup(linuxUser) {
    ...
    if (adapter) {
      try {
        ...
        // Capture the SDK-reported active model per vendor so it can be
        // surfaced to the client on model_info payloads (issue #22).
        sm.defaultModelByVendor = sm.defaultModelByVendor || {};
        if (result.defaultModel) {
          sm.defaultModelByVendor[defaultVendor] = result.defaultModel;
        }
    ...
    send({
      ...
      defaultModel: (sm.defaultModelByVendor && sm.defaultModelByVendor[defaultVendor]) || "",
      availableVendors: sm.availableVendors,
      installedVendors: sm.installedVendors,
    });
  }
